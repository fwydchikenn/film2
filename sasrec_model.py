...
class SASRecWithLLMAndIPS(nn.Module):
    """SASRec WITH LLM + IPS (Proposed)"""
    def __init__(self, n_items, embedding_dim=128, n_heads=2, n_layers=2,
                 max_seq_len=50, dropout=0.2, llm_dim=768):
        super(SASRecWithLLMAndIPS, self).__init__()
    ...
    def forward(self, item_seq, llm_features):
        ...
        # Fuse with LLM features using gating
        ...
        for i in range(len(self.attention_layers)):
            residual = seq_emb
            seq_emb = self.layer_norms1[i](seq_emb)
            seq_emb, _ = self.attention_layers[i](
                seq_emb, seq_emb, seq_emb, attn_mask=attn_mask
            )
            seq_emb = residual + self.dropout(seq_emb)

            residual = seq_emb
            seq_emb = self.layer_norms2[i](seq_emb)
